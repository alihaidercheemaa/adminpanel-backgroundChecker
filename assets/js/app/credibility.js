let manageCredibility = () => {
    let image = $('.image').val();
    if (image == "") {
        Notification('danger', 'Error', `Please upload credibility image`);
        return false;
    }
    let data = {
        image: image,
        requestType: 'manageCredibility'
    };
    $.post('datafiles/credibility', data, function (response) {
        let result = JSON.parse(response);
        if (result.status == 1) {
            Notification('success', 'Success', `${result.message}`);
            $('#credibilityList').DataTable().ajax.reload();
...
let credibilityInit = () => {
    $("#credibilityList").DataTable({
        ...
        ajax: {
            url: "./datafiles/credibility",
            type: "POST",
            data: {
                requestType: 'credibilityList'
            },
        },
