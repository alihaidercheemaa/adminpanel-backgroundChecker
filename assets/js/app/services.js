    ...
    let manageServices = () => {
        let forms = $('.services_form');
        let image = $('.image').val();
        if(image == ""){
            Notification('danger', 'Error', `Please upload service image`);
            return false;
        }
        let fields = ['title', 'description'];
        let module = $('#manageServices').data("module");
        let isValid = true;
        forms.each(function () {
        ...
        if (isValid) {
            let data = {
                ...
                module: module,
                requestType: 'manageServices'
            };
            if (module == 'update') {
                data.id = $('#manageServices').attr("services_id");
            }
            $.post('datafiles/services', data, function (response) {
    ...
    $(document).ready(function () {
        ...
        servicesInit(start, end, '');
    });
    function servicesInit(start, end, label) {
        var title = '';
        var range = '';
        var rangejson = '';
        if (label == 'Today') {
        ...
        let servicesList = () => {
            ...
            buttons: [
                ...
                ajax: {
                    url: "./datafiles/services",
                    type: "POST",
                    data: {
                        start_date: start_date,
                        end_date: end_date,
                        requestType: 'servicesList'
                    },
                },
