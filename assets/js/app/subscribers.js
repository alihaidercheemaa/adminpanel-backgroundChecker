...
//                     .then(() => {
//                         window.location.href = "/subscriberList";
//                     });
//             } else {
...
const subscriberList = () => {
    $("#subscriberList").DataTable({
        ...
        ajax: {
            url: "./datafiles/subscribers",
            type: "POST",
            data: {
                requestType: 'subscriberList'
            },
        },
...
let subscribeEmail = () => {
    ...
    if (message == "") {
        Notification('danger', 'Error', `Please Enter Message`);
        return false;
    }
    let data = {
        message: message,
        requestType: 'subscribeEmail'
    };
    $.post('datafiles/subscribers', data, function (response) {
        let result = JSON.parse(response);
        if (result.status == 1) {
