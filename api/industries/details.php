<?php
    try {
        ...
        $query = "SELECT `id`, `title`, `slug`, `description`, `image`,  DATE_FORMAT(created_date, '%M %d, %Y') AS `created_date` FROM `industries` WHERE `id` = '$request->id' AND `is_deleted`='0' ORDER BY `id` DESC LIMIT 1"; 
        $dbobjx->query($query);
