<?php
    try {
        ...
        $query = "SELECT COUNT(`id`) AS `row_count` FROM `industries_details` WHERE `title` = '$title' AND `id` != '$id'"; 
        $dbobjx->query($query);
        if(($dbobjx->single($query))->row_count > 0){
            echo response("0", "Duplicate industry title found, Please try something else.", []);
            exit();
        }
        $query = "UPDATE `industries_details` SET `title`='$title',`description`='$description',`image`='$image',`updated_date`=NOW() WHERE `id` = '$id'"; 
        $dbobjx->query($query);
